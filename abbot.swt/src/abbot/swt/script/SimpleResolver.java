...
public class SimpleResolver extends AbstractResolver {
	...
	public SimpleResolver(WidgetHierarchy hierarchy) {
		super(hierarchy);
	}
	...
	public WidgetReference getWidgetReference(String name) {
		synchronized (refs) {
			Log.debug("Looking for reference with id '" + name + "' among " + refs.size()
					+ " references");
	...
	public WidgetReference getWidgetReference(Widget widget) {
		synchronized (refs) {
			WidgetReference match = (WidgetReference) widgets.get(widget);
//			if (match == null) {
//				/* want to get a widget reference from a given widget */
//				match = DefaultWidgetFinder.getFinder().matchWidget(
//						widget,
...
//			}
	...
	public WidgetReference addWidget(Widget widget) {
		...
		synchronized (refs) {
			WidgetReference ref = getWidgetReference(widget);
			/* references are being used here to track widgets */
			if (ref == null) {
				// Nope, create a new one
				/*
				 * Log.debug("Component " + ComponentTester.toString(comp) + " not yet referenced,
				 * adding it");
				 */
				if (widget == null) {
					System.out.print("");
				...
				/*
				 * Log.debug("Component " + ComponentTester.toString(comp) + " already referenced,
				 * using " + ref);
				 */
			}
			return ref;
	...
	public String getUniqueID(WidgetReference ref) {
		...
		if (id == null) {
			...
			if (ref.getName() != null && !"".equals(ref.getName())) {
				id = ref.getName();
			} else if (ref.getTag() != null && !"".equals(ref.getTag())) {
				id = ref.getTag();
			} else {
				id = cname + " Instance";
			}
