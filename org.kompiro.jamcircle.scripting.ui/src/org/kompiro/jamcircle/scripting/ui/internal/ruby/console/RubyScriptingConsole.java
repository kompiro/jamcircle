...
package org.kompiro.jamcircle.scripting.ui.internal.ruby.console;
...
import org.kompiro.jamcircle.scripting.ui.internal.ui.console.*;
...
public class RubyScriptingConsole extends TextConsole implements IScriptingConsole {
	...
	private IScriptingIOConsolePage consolePage;

	// private Ruby runtime;
	private ScriptingContainer container;

	private ScriptingConsoleOutputStream output;

	private ScriptingConsoleOutputStream error;

	private ScriptingConsoleInputStream input;
	...
	public RubyScriptingConsole(String name, String consoleType, ImageDescriptor imageDescriptor, String encoding,
		...
		inputStream = new ScriptingConsoleInputStream(this);
		synchronized (openStreams) {
			openStreams.add(inputStream);
		}

		partitioner = new ScriptingConsolePartitioner(inputStream, this);
	...
	public IPageBookViewPage createPage(IConsoleView view) {
		consolePage = new IScriptingIOConsolePage(this, view);
	 ...
	 */
	public ScriptingConsoleOutputStream newOutputStream() {
		ScriptingConsoleOutputStream outputStream = new ScriptingConsoleOutputStream(this);
	 ...
	 */
	public ScriptingConsoleInputStream getInputStream() {
		return inputStream;
	}
	 ...
	 *            stream that closed
	 */
	public void streamClosed(ScriptingConsoleOutputStream stream) {
		synchronized (openStreams) {
	 ...
	 *            stream that closed
	 */
	public void streamClosed(ScriptingConsoleInputStream stream) {
		synchronized (openStreams) {
			openStreams.remove(stream);
			checkFinished();
		}
	...
	protected void dispose() {
		...
		for (int i = 0; i < allStreams.length; i++) {
			...
			if (stream instanceof ScriptingConsoleInputStream) {
				ScriptingConsoleInputStream is = (ScriptingConsoleInputStream) stream;
				try {
					is.close();
				} catch (IOException e) {
				}
			} else if (stream instanceof ScriptingConsoleOutputStream) {
				ScriptingConsoleOutputStream os = (ScriptingConsoleOutputStream) stream;
				try {
					os.close();
				} catch (IOException e) {
				}
	...
	private void createJob() {
		...
		Job reader = new Job(Messages.RubyScriptingConsole_run_irb_message) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				String script = IOUtils.getStringFromResource(RubyScriptingConsole.class, INIT_SCRIPT_RB);
	...
	private final class HandleHistoryAndCompletionListener implements VerifyKeyListener, ICompletionListener,
		...
		private boolean isReadOnly(int nextCaretOffset) {
			...
			if (partition instanceof IScriptingIOConsolePartition) {
				IScriptingIOConsolePartition ioPartition = (IScriptingIOConsolePartition) partition;
		...
		private void write(String oldLine) {
			try {
				...
				int lastOffset;
				if (IScriptingIOConsolePartition.OUTPUT_PARTITION_TYPE.equals(partition.getType())) {
					lastOffset = last;
