...
public class ScriptingConsolePartitioner implements IConsoleDocumentPartitioner, IDocumentPartitionerExtension {
	...
	public String[] getLegalContentTypes() {
		return new String[] { IScriptingIOConsolePartition.OUTPUT_PARTITION_TYPE, IScriptingIOConsolePartition.INPUT_PARTITION_TYPE };
	}
	...
	public IRegion documentChanged2(DocumentEvent event) {
		...
		if (document.getLength() == 0) { //document cleared
			if (lastPartition != null && lastPartition.getType().equals(IScriptingIOConsolePartition.INPUT_PARTITION_TYPE)) {
				synchronized (partitions) {
				...
				if (lastLineDelimiter != -1) {
					...
					while (it.hasNext()) {
					    ...
					    if (partition.getOffset() + partition.getLength() <= event.fOffset + lastLineDelimiter) {
					        ...
					        IScriptingIOConsolePartition newPartition = new IScriptingIOConsolePartition(inputStream, contentBefore);
    ...
    public StyleRange[] getStyleRanges(int offset, int length) {
        ...
        IScriptingIOConsolePartition[] computedPartitions = (IScriptingIOConsolePartition[])computePartitioning(offset, length);
