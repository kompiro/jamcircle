...
public class BoardLocalLayout {
	...
	private EditPart getCardEditPart(Rectangle target) {
		...
		for (Entry<?, ?> entry : visualPartMap.entrySet()) {
			if ((entry.getKey() instanceof AnnotationArea<?>) == false)
				continue;
			AnnotationArea<?> area = (AnnotationArea<?>) entry.getKey();
			Object targetFig = area.getTargetFigure();
			if ((targetFig instanceof CardFigure) == false)
				continue;
			...
			if (isTouchedTargetCard(target, cardRect)) {
				...
				if (editPart instanceof CardEditPart)
					return editPart;
			}
		}
		return null;
