...
public class KanbanUIActivator extends AbstractUIPlugin {
	...
	private final class MessageDialogStatusHandler implements IStatusHandler {
		...
		public void fail(final IStatus status, boolean informUser) {
			...
			if (informUser && !getKanbanService().isTestMode()) {
				WorkbenchUtil.async(new Runnable() {
					public void run() {
						String message = format(Messages.KanbanUIActivator_error_message, status.getMessage(),
								exception
										.getClass().getName(), exception.getLocalizedMessage());
	...
	public KanbanService getKanbanService() {
		...
		if (context == null)
			throw new IllegalStateException(Messages.KanbanUIActivator_error_kanban_context);
		KanbanService service = context.getKanbanService();
		if (service == null)
			throw new IllegalStateException(Messages.KanbanUIActivator_error_kanban_service);
		service.init();
		return service;
	}
	...
	public ScriptingService getScriptingService() throws ScriptingException {
		...
		if (service == null)
			throw new IllegalStateException(Messages.KanbanUIActivator_error_scripting_service);
		return service;
	}
