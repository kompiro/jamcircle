...
public class XMPPConnectionServiceImpl implements XMPPConnectionService {
			...
			Job job = new Job(message) {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					try {
						...
						XMPPStatusHandler.debug(Messages.XMPPConnectionServiceImpl_initialize_connection_error_message,
								e);
					} finally {
						monitor.done();
					}
	...
	public void login(IProgressMonitor monitor,
		...
		XMPPStatusHandler.debug(
				"createConnection: host:'%s' resource:'%s' serviceName:'%s' port:'%d' username:'%s' password'%s'" //$NON-NLS-1$
